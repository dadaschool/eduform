...
2. **업스테이지를 쓴다** — `UPSTAGE_API_KEY` 를 등록합니다.
   업스테이지는 **2027-04-30 까지 무료**입니다. 그 뒤로는 돈이 나갑니다 —
   아래 [유료 AI 로 넘어가기 전에 묻는다] 를 읽으세요.
...
- `src/lib/ai.ts` 의 `PRICING` 표가 제공자마다 «언제부터 돈이 나가는가» 를 들고 있습니다.
  **유료인 동안에는 `allowPaid` 허락 없이 호출되지 않습니다.**
- **요금은 시점에 따라 다릅니다.** 업스테이지는 `2027-04-30` 까지 무료라서, 지금은
  Gemini 가 실패하면 **묻지 않고** 넘어갑니다. 그 날짜가 지나면 **코드를 고치지 않아도**
  다시 확인을 묻습니다. 헛되게 매번 묻는 창은 결국 눈감고 누르게 만들고, 정작 돈이
  나갈 때의 경고까지 무력해집니다.
...
⚠ **제공자를 새로 추가하면 `PRICING` 표에 반드시 적을 것.** 빠뜨리면 무료로 취급되어
다시 조용히 과금됩니다.
⚠ **`AI_PRIMARY=upstage`** — 무료 기간에는 문제없습니다. `2027-05-01` 이후에는 유료를
1순위로 두는 설정이 되어 버튼을 누를 때마다 확인 창이 뜹니다(막히지는 않습니다).

확인은 `npm run test:ai-gate` 로 합니다. 무료 기간과 유료 기간 양쪽을 시험합니다 —
`generateText({ at })` 로 시각을 넣을 수 있어 2027년을 기다리지 않아도 됩니다.
